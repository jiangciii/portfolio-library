...
run('archive sections expose the four requested anchors', () => {
  assert.deepEqual(
...
run('loose-leaf tabs expose numbered section labels', () => {
...
run('each section includes divider copy for chapter pages', () => {
...
run('design categories include All first and unique work categories after it', () => {
...
run('filterDesignWorks returns all items for All and narrows by category', () => {
...
run('getResearchProject finds expandable research detail by slug', () => {
